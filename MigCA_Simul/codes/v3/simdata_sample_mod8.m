...
for x = 1:size(sampsize, 2)
    ...
    for t = 1:T
        ...
        for m = 1:M
            choices(m, :) = vec(permute(welf_ijkt(origin(m), :, :, t), [3 2 1]))';
        end
    ...
    numberppl_vec = vec(permute(numberppl, [4 3 2 1]))
    ...
    simulated_sampled = [ones(size(sample_earn_vec, 1), 1).*x dims ...
        sample_earn_vec sample_L_ijkt_vec numberppl_vec sample_L_iktlag_vec ...
...
% Exporting data
csvwrite('../../outputs/simulated_sampled_all_mod8.csv', simulated_sampled_all);
