...
merge destination crop using "../../data/output/w_jkt_za_07"
sort origin destination crop
drop _merge year

* variables in logs
gen L_iktlag_log = log(L_iktlag)
gen w_jkt_log = log(w_jkt)
...
* Regression (10), OLS
reghdfe w_jkt_log L_iktlag_log if origin != destination & w_jkt_log != ., absorb(iota_jkt iota_ijt) vce(cluster iota_jkt)
estimates store reg11_70lag80_ols

* Regression (10), PPML
ppmlhdfe w_jkt L_iktlag_log if origin != destination, absorb(iota_jkt iota_ijt) vce(cluster iota_jkt)
